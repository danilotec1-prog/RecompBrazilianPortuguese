...
Regenerate supplementary TXT edits with `python text_converter/extra_converter.py` from the repository root before building. Running `text_converter/converter.py` from its directory also regenerates them. See [editing instructions](docs/EDITING_TEXTS.md).
